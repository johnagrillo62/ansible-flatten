(playbook "kubespray/roles/kubernetes-apps/csi_driver/upcloud/defaults/main.yml"
  ...
  (upcloud_username (jinja "{{ lookup('env', 'UPCLOUD_USERNAME') }}"))
  (upcloud_password (jinja "{{ lookup('env', 'UPCLOUD_PASSWORD') }}"))
  (upcloud_tolerations (list))
  ...
  (upcloud_cacert (jinja "{{ lookup('env', 'OS_CACERT') }}")))
