(playbook "debops/ansible/roles/apt_preferences/tasks/main.yml"
  (tasks
    (task "Remove legacy APT preferences"
      (ansible.builtin.file 
        (dest "/etc/apt/preferences.d/" (jinja "{{ item.filename | d((item.role | d(item.by_role | d(\"pin\")))
        ...
        (state "absent"))
      (loop (jinja "{{ q(\"flattened\", apt_preferences_list | d([])
    ...
    (task "Remove APT preferences"
      (ansible.builtin.file 
        (dest "/etc/apt/preferences.d/" (jinja "{{ item.filename | d((item.role | d(item.by_role | d(\"pin\"))) | replace(\".\", \"_\")
                                     + \"_\" + ((item.package.split(\" \")[0]
                                               if item.package is string
                                               else item.package[0])
                                              if item.package | d()
