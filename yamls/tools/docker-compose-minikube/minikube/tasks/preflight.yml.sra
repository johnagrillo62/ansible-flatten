(playbook "tools/docker-compose-minikube/minikube/tasks/preflight.yml"
  (tasks
    (task "Check if Kubernetes python module is installed"
      (shell "pip freeze | grep openshift")
      (register "_pip_openshift")
      (ignore_errors "true"))
    (task "Preflight check - require openshift python module"
      (assert 
