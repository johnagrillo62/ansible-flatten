(playbook "debops/ansible/roles/roundcube/defaults/main.yml"
  (roundcube__required_php_packages (list
      "iconv"
      "openssl"
      "session"
      "sockets"
      ...
      "json"
      "intl"))
  (roundcube__optional_php_packages (list
      (jinja "{{ \"ldap\" if roundcube__ldap_enabled | bool else [] }}")
      (jinja "{{ \"memcached\" if roundcube__memcached_enabled | bool else [] }}")
      (jinja "{{ \"redis\" if roundcube__redis_enabled | bool else [] }}")
      "fileinfo"
      "enchant"
      "zip"))
  (roundcube__custom_php_packages (list))
  (roundcube__base_php_packages (list
      (jinja "{{ roundcube__required_php_packages }}")
      (jinja "{{ roundcube__apt_php_packages }}")
      (jinja "{{ roundcube__optional_php_packages }}")
      (jinja "{{ [\"mysql\"] if (roundcube__database_map[roundcube__database].dbtype == \"mysql\") else [] }}")
      (jinja "{{ [\"pgsql\"] if (roundcube__database_map[roundcube__database].dbtype == \"postgresql\") else [] }}")
      (jinja "{{ [\"sqlite3\"] if (roundcube__database_map[roundcube__database].dbtype == \"sqlite\") else [] }}")))
  (roundcube__apt_php_packages (jinja "{{ [\"mail-mime\", \"net-smtp\", \"pear\"]
                                 if ansible_distribution_release in [\"stretch\", \"buster\", \"sid\", \"xenial\", \"yakkety\", \"zesty\", \"artful\"]
                                 else [] }}"))
  (roundcube__packages (list))
  (roundcube__base_packages (list
      "curl"
      "file"
      "unzip"
      "aspell"
  ...
  (roundcube__home (jinja "{{ (ansible_local.fhs.home | d(\"/var/local\"))
                     + \"/\" + roundcube__user }}"))
  ...
  (roundcube__git_additional_gpg_keys (list))
  (roundcube__git_repo "https://github.com/roundcube/roundcubemail.git")
  (roundcube__git_dir (jinja "{{ roundcube__src + \"/\"
                        + roundcube__git_repo.split(\"@\"
                                                    if (\"@\" in roundcube__git_repo)
                                                    else \"://\")[1] }}"))
  (roundcube__git_version "1.6.0")
  (roundcube__git_dest (jinja "{{ roundcube__www + \"/sites/\" + roundcube__user + \"/public\" }}"))
  (roundcube__src (jinja "{{ (ansible_local.fhs.src | d(\"/usr/local/src\"))
                    + \"/\" + roundcube__user }}"))
  (roundcube__www (jinja "{{ ansible_local.nginx.www
                    if (ansible_local | d() and ansible_local.nginx | d())
                    else \"/srv/www\" }}"))
  (roundcube__webserver_user (jinja "{{ ansible_local.nginx.user | d(\"www-data\") }}"))
  (roundcube__database "sqlite-default")
  (roundcube__database_user "roundcube")
  (roundcube__database_password_path (jinja "{{ secret + \"/credentials/\" + inventory_hostname
                                       + \"/roundcube/\" + roundcube__database
                                       + \"/\" + roundcube__database_user + \"/password\" }}"))
  (roundcube__database_password (jinja "{{ lookup(\"password\", roundcube__database_password_path + \" length=30\") }}"))
  (roundcube__database_name (jinja "{{ roundcube__database_user
  ...
  (roundcube__database_map 
    ...
    (mysql-default 
      (dbtype "mysql")
      (dbname (jinja "{{ roundcube__database_name }}"))
      (dbuser (jinja "{{ roundcube__database_user }}"))
      (dbpass (jinja "{{ roundcube__database_password }}"))
      (dbhost "localhost")
      (dbtableprefix ""))
    (postgresql-default 
      (dbtype "postgresql")
  ...
  (roundcube__database_schema_map 
    (mysql (jinja "{{ roundcube__git_dest + \"/SQL/mysql.initial.sql\" }}"))
    (postgresql (jinja "{{ roundcube__git_dest + \"/SQL/postgres.initial.sql\" }}")))
  (roundcube__database_schema (jinja "{{ roundcube__database_schema_map[roundcube__database_map[roundcube__database].dbtype]
                                if roundcube__database_schema_map[roundcube__database_map[roundcube__database].dbtype] | d()
                                else \"\" }}"))
  (roundcube__memcached_enabled (jinja "{{ True
                                  if (ansible_local | d() and ansible_local.memcached | d() and
                                      (ansible_local.memcached.installed | d()) | bool)
                                  else False }}"))
  (roundcube__memcached_hosts (list
      "localhost:11211"))
  (roundcube__redis_enabled (jinja "{{ True
  ...
  (roundcube__redis_port (jinja "{{ ansible_local.redis_server.port | d(\"6379\") }}"))
  (roundcube__redis_database "1")
  (roundcube__redis_password (jinja "{{ ansible_local.redis_server.password | d(\"\") }}"))
  (roundcube__redis_hosts (list
      (jinja "{{ ([roundcube__redis_server,
                                roundcube__redis_port,
                                roundcube__redis_database,
                                roundcube__redis_password])
                              | join(\":\") }}")))
  (roundcube__session_storage (jinja "{{ \"redis\"
                                if roundcube__redis_enabled | bool
                                else (\"memcached\"
                                      if roundcube__memcached_enabled | bool
                                      else \"\") }}"))
  (roundcube__ldap_enabled (jinja "{{ ansible_local.ldap.enabled
                             if (ansible_local | d() and ansible_local.ldap | d() and
                                 ansible_local.ldap.enabled is defined)
                             else False }}"))
  (roundcube__ldap_password_enabled (jinja "{{ roundcube__ldap_enabled }}"))
  (roundcube__ldap_addressbook_name "LDAP Address Book")
  (roundcube__ldap_hosts (jinja "{{ ansible_local.ldap.hosts | d([]) }}"))
  (roundcube__ldap_port (jinja "{{ ansible_local.ldap.port | d(\"389\") }}"))
  (roundcube__ldap_use_tls (jinja "{{ ansible_local.ldap.start_tls | d(True) }}"))
  (roundcube__ldap_base_dn (jinja "{{ ansible_local.ldap.base_dn | d([]) }}"))
  (roundcube__ldap_device_dn (jinja "{{ ansible_local.ldap.device_dn | d([]) }}"))
  (roundcube__ldap_self_rdn (jinja "{{ \"uid=\" + roundcube__user }}"))
  (roundcube__ldap_self_object_classes (list
      "account"
      "simpleSecurityObject"))
  (roundcube__ldap_self_attributes 
    (uid (jinja "{{ roundcube__ldap_self_rdn.split(\"=\")[1] }}"))
    (userPassword (jinja "{{ roundcube__ldap_bindpw }}"))
  ...
  (roundcube__ldap_binddn (jinja "{{ ([roundcube__ldap_self_rdn] + roundcube__ldap_device_dn) | join(\",\") }}"))
  (roundcube__ldap_bindpw (jinja "{{ (lookup(\"password\", secret + \"/ldap/credentials/\"
                                    + roundcube__ldap_binddn | to_uuid + \".password length=32\"))
                            if roundcube__ldap_enabled | bool
                            else \"\" }}"))
  (roundcube__ldap_people_rdn (jinja "{{ ansible_local.ldap.people_rdn | d([]) }}"))
  (roundcube__ldap_people_dn (jinja "{{ [roundcube__ldap_people_rdn]
                               + roundcube__ldap_base_dn }}"))
  (roundcube__ldap_groups_rdn (jinja "{{ ansible_local.ldap.groups_rdn | d([]) }}"))
  (roundcube__ldap_groups_dn (jinja "{{ [roundcube__ldap_groups_rdn]
                               + roundcube__ldap_base_dn }}"))
  ...
  (roundcube__ldap_field_map 
    ...
    (notes "description:*")
    (photo "jpegPhoto"))
  (roundcube__fqdn "webmail." (jinja "{{ roundcube__domain }}"))
  (roundcube__domain (jinja "{{ ansible_domain }}"))
  (roundcube__imap_srv_rr (jinja "{{ q(\"debops.debops.dig_srv\", \"_imaps._tcp.\" + roundcube__domain,
                              ansible_fqdn
                              if (ansible_local | d() and ansible_local.dovecot | d() and
                                  (ansible_local.dovecot.installed | d()) | bool)
                              else \"imap.\" + roundcube__domain, 993) }}"))
  (roundcube__imap_fqdn (jinja "{{ roundcube__imap_srv_rr[0][\"target\"] }}"))
  (roundcube__imap_port (jinja "{{ roundcube__imap_srv_rr[0][\"port\"] }}"))
  (roundcube__imap_server (jinja "{{ (\"ssl://\"
  ...
  (roundcube__smtp_srv_rr (jinja "{{ q(\"debops.debops.dig_srv\", \"_submissions._tcp.\" + roundcube__domain,
                              ansible_fqdn
                              if (ansible_local | d() and ansible_local.postfix | d() and
                                  (ansible_local.postfix.installed | d()) | bool)
                              else (\"smtp.\" + roundcube__domain), 465) }}"))
  (roundcube__smtp_fqdn (jinja "{{ roundcube__smtp_srv_rr[0][\"target\"] }}"))
  (roundcube__smtp_port (jinja "{{ roundcube__smtp_srv_rr[0][\"port\"] }}"))
  (roundcube__smtp_server (jinja "{{ (\"ssl://\"
  ...
  (roundcube__sieve_srv_rr (jinja "{{ q(\"debops.debops.dig_srv\", \"_sieve._tcp.\" + roundcube__domain,
                               ansible_fqdn
                               if (ansible_local | d() and ansible_local.dovecot | d() and
                                   (ansible_local.dovecot.installed | d()) | bool)
                               else (\"sieve.\" + roundcube__domain), 4190) }}"))
  (roundcube__sieve_fqdn (jinja "{{ roundcube__sieve_srv_rr[0][\"target\"] }}"))
  (roundcube__sieve_port (jinja "{{ roundcube__sieve_srv_rr[0][\"port\"] }}"))
  (roundcube__sieve_server (jinja "{{ \"tls://\" + roundcube__sieve_fqdn }}"))
  (roundcube__product_name (jinja "{{ ansible_local.machine.organization | d(\"Roundcube\") + \" Webmail\" }}"))
  (roundcube__des_key (jinja "{{ lookup(\"password\", secret + \"/credentials/\" + inventory_hostname + \"/roundcube/des_key chars=hexdigits length=24\") }}"))
  (roundcube__username_domain "%d")
  (roundcube__log_driver "syslog")
  (roundcube__original_configuration (list
...
")
      ...
      (name "db_dsnw")
      (comment "Database connection string (DSN) for read+write operations
...
")
      ...
      (name "db_dsnr")
      (comment "Database DSN for read-only operations (if empty write database will be used)
...
")
      ...
      (name "db_persistent")
      (comment "use persistent db-connections
...
")
      ...
      (name "db_table_dsn")
      (comment "Mapping of table names and connections to use for ALL operations.
...
")
      ...
      (comment "It is possible to specify database variable values e.g. some limits here.
...
")
      ...
      (name "log_driver")
      (comment "Log driver: 'syslog', 'stdout' or 'file'.
")
      (value "file")
      (section "logging")
      (state "init")
...
")
      ...
      (name "log_session_id")
      (comment "Length of the session ID to prepend each log line with
...
")
      ...
      (comment "Syslog facility to use, if using the 'syslog' log driver.
...
")
      ...
      (comment "Activate this option if logs should be written to per-user directories.
...
")
      ...
      (name "session_debug")
      (comment "Log session debug information/authentication errors to <log_dir>/session.log or to syslog
")
      (value "False")
      (section "logging")
      (state "init")
      ...
      (name "imap_host")
      (comment "IMAP host chosen to perform the log-in.
...
")
      ...
      (name "imap_auth_type")
      (comment "IMAP authentication method (DIGEST-MD5, CRAM-MD5, LOGIN, PLAIN or null).
...
")
      ...
      (comment "IMAP socket context options
...
")
      ...
      (name "imap_delimiter")
      (comment "If you know your imap's folder delimiter, you can specify it here.
...
")
      ...
      (name "imap_vendor")
      (comment "If you know your imap's folder vendor, you can specify it here.
...
")
      ...
      (comment "If IMAP server doesn't support NAMESPACE extension, but you're
...
")
      ...
      (comment "By default IMAP capabilities are read after connection to IMAP server
...
")
      ...
      (comment "By default list of subscribed folders is determined using LIST-EXTENDED
...
")
      ...
      (name "imap_force_ns")
      (comment "Some server configurations (e.g. Courier) doesn't list folders in all namespaces
...
")
      ...
      (comment "Some servers return hidden folders (name starting with a dot)
...
")
      ...
      (comment "Some servers do not support folders with both folders and messages inside
...
")
      ...
      (comment "List of disabled imap extensions.
...
")
      ...
      (comment "Log IMAP session identifiers after each IMAP login.
...
")
      ...
      (name "imap_cache")
      (comment "Type of IMAP indexes cache. Supported values: 'db', 'apc' and 'memcache' or 'memcached'.
")
      (value null)
      (section "imap")
      (state "init")
      ...
      (name "messages_cache")
      (comment "Enables messages cache. Only 'db' cache is supported.
...
")
      ...
      (comment "Maximum cached message size in kilobytes.
...
")
      ...
      (name "smtp_host")
      (comment "SMTP server host (for sending mails).
See defaults.inc.php for the option description.
")
      (value "localhost:587")
      (section "smtp")
      ...
      (name "smtp_user")
      (comment "SMTP username (if required) if you use %u as the username Roundcube
...
")
      ...
      (name "smtp_pass")
      (comment "SMTP password (if required) if you use %p as the password Roundcube
...
")
      ...
      (name "smtp_auth_type")
      (comment "SMTP AUTH type (DIGEST-MD5, CRAM-MD5, LOGIN, PLAIN or empty to use
...
")
      ...
      (name "smtp_auth_cid")
      (comment "Optional SMTP authentication identifier to be used as authorization proxy
")
      (value null)
      (section "smtp")
      (state "init")
      ...
      (name "smtp_auth_pw")
      (comment "Optional SMTP authentication password to be used for smtp_auth_cid
")
      (value null)
      (section "smtp")
      (state "init")
...
")
      ...
      (name "smtp_timeout")
      (comment "SMTP connection timeout, in seconds. Default: 0 (use default_socket_timeout)
...
")
      ...
      (comment "SMTP socket context options
See https://php.net/manual/en/context.ssl.php
...
")
      ...
      (name "ldap_cache")
      (comment "Type of LDAP cache. Supported values: 'db', 'apc' and 'memcache' or 'memcached'.
")
      (value "db")
      (section "ldap")
      (state "init")
      ...
      (name "memcache_hosts")
      (comment "Use these hosts for accessing memcached
...
")
      ...
      (comment "Controls the use of a persistent connections to memcache servers
...
")
      ...
      (comment "Value in seconds which will be used for connecting to the daemon
See https://php.net/manual/en/memcache.addserver.php
")
      (value "1")
      (section "cache")
      ...
      (comment "Controls how often a failed server will be retried (value in seconds).
...
")
      ...
      (name "redis_hosts")
      (comment "Use these hosts for accessing Redis.
...
")
      ...
      (comment "THIS OPTION WILL ALLOW THE INSTALLER TO RUN AND CAN EXPOSE SENSITIVE CONFIG DATA.
...
")
      ...
      (name "advanced_prefs")
      (comment "Define which settings should be listed under the 'advanced' block
...
")
      ...
      (name "support_url")
      (comment "Provide an URL where a user can get support for this Roundcube installation
...
")
      ...
      (name "skin_logo")
      (comment "Logo image replacement. Specifies location of the image as:
...
")
      ...
      (comment "Automatically create a new Roundcube user when log-in the first time.
...
")
      ...
      (name "log_dir")
      (comment "use this folder to store log files
...
")
      ...
      (name "temp_dir")
      (comment "Use this folder to store temp files
Must be writeable for the user who runs PHP process (Apache user if mod_php is being used)
")
      ...
      (name "temp_dir_ttl")
      (comment "Expire files in temp_dir after 48 hours
...
")
      ...
      (name "force_https")
      (comment "Enforce connections over https
...
")
      ...
      (name "use_https")
      (comment "Tell PHP that it should work as under secure connection
...
")
      ...
      (comment "Allow browser-autocompletion on login form.
...
")
      ...
      (name "login_lc")
      (comment "Forces conversion of logins to lower case.
...
")
      ...
      (comment "Logon username filter. Regular expression for use with preg_match().
Example: '/^[a-z0-9_@.-]+$/'
")
      (value null)
      (section "system")
      ...
      (comment "Brute-force attacks prevention.
...
")
      ...
      (comment "Display product name and software version on login screen
...
")
      ...
      (name "session_name")
      (comment "Session name. Default: 'roundcube_sessid'
")
      (value null)
      (section "system")
      (state "init")
      ...
      (comment "Session authentication cookie name. Default: 'roundcube_sessauth'
")
      (value null)
      (section "system")
      (state "init")
      ...
      (name "session_path")
      (comment "Session path. Defaults to PHP session.cookie_path setting.
")
      (value null)
      (section "system")
      (state "init")
      ...
      (comment "Backend to use for session storage. Can either be 'db' (default), 'redis', 'memcache', or 'php'
...
")
      ...
      (comment "List of trusted proxies
...
")
      ...
      (comment "List of trusted host names
...
")
      ...
      (comment "X-Frame-Options HTTP header value sent to prevent from Clickjacking.
...
")
      ...
      (name "des_key")
      (comment "this key is used to encrypt the users imap password which is stored
...
")
      ...
      (name "cipher_method")
      (comment "Encryption algorithm. You can use any method supported by OpenSSL.
...
")
      ...
      (comment "Automatically add this domain to user names for login
...
")
      ...
      (comment "Force domain configured in username_domain to be used for login.
...
")
      ...
      (name "mail_domain")
      (comment "This domain will be used to form e-mail addresses of new users
Specify an array with 'host' => 'domain' values to support multiple hosts
Supported replacement variables:
%h - user's IMAP hostname
...
")
      ...
      (comment "Password character set, to change the password for user
...
")
      ...
      (comment "Message size limit. Note that SMTP server(s) may use a different value.
...
")
      ...
      (name "max_recipients")
      (comment "Maximum number of recipients per message (including To, Cc, Bcc).
Default: 0 (no limit)
")
      (value "0")
      (section "system")
      ...
      (comment "Maximum number of recipients per message excluding Bcc header.
...
")
      ...
      (comment "Maximum allowed number of members of an address group. Default: 0 (no limit)
...
")
      ...
      (name "product_name")
      (comment "Name your service. This is displayed on the login screen and in the window title
")
      ...
      (comment "Try to load host-specific configuration
...
")
      ...
      (comment "Path to a text file which will be added to each sent message
...
")
      ...
      (comment "Path to a text file which will be added to each sent HTML message
Paths are relative to the Roundcube root folder
")
      (value "")
      (section "system")
      ...
      (comment "Add a received header to outgoing mails containing the creators IP and hostname
")
      (value "False")
      (section "system")
      (state "init")
      ...
      (comment "Whether or not to encrypt the IP address and the host name
...
")
      ...
      (name "line_length")
      (comment "Number of chars allowed for line when wrapping text.
...
")
      ...
      (name "mdn_use_from")
      (comment "According to RFC2298, return receipt envelope sender address must be
...
")
      ...
      (comment "Set identities access level:
...
")
      ...
      (comment "Maximum size of uploaded image in kilobytes
...
")
      ...
      (comment "Mimetypes supported by the browser.
...
")
      ...
      (name "mime_magic")
      (comment "Path to a local mime magic database file for PHPs finfo extension.
...
")
      ...
      (name "mime_types")
      (comment "Absolute path to a local mime.types mapping table file.
...
")
      ...
      (comment "Path to imagemagick identify binary (if not set we'll use Imagick or GD extensions)
")
      (value null)
      (section "system")
      (state "init")
      ...
      (comment "Path to imagemagick convert binary (if not set we'll use Imagick or GD extensions)
")
      (value null)
      (section "system")
      (state "init")
      ...
      (comment "Size of thumbnails from image attachments displayed below the message content.
...
")
      ...
      (comment "Disables saving sent messages in Sent folder (like gmail) (Default: false)
...
")
      ...
      (comment "Improve system security by using special URL with security token.
...
")
      ...
      (name "assets_path")
      (comment "Allows to define separate server/path for image/js/css files
...
")
      ...
      (name "assets_dir")
      (comment "While assets_path is for the browser, assets_dir informs
...
")
      ...
      (comment "Default messages sort column. Use empty value for default server's sorting,
...
")
      ...
      (name "list_cols")
      (comment "These cols are shown in the message list. Available cols are:
...
")
      ...
      (name "language")
      (comment "The default locale setting (leave empty for auto-detection)
...
")
      ...
      (name "date_formats")
      (comment "Give this choice of date formats to the user to select from
...
")
      ...
      (name "date_short")
      (comment "Use this format for short date display (derived from date_format and
time_format)
")
      (value "D H:i")
      (section "ui")
      ...
      (name "date_long")
      (comment "Use this format for detailed date/time formatting (derived from
...
")
      ...
      (name "drafts_mbox")
      (comment "Store draft message is this mailbox
...
")
      ...
      (name "junk_mbox")
      (comment "Store spam messages in this mailbox
NOTE: Use folder names with namespace prefix (INBOX. on Courier-IMAP)
")
      (value "Junk")
      (section "ui")
      ...
      (name "sent_mbox")
      (comment "Store sent message is this mailbox
...
")
      ...
      (name "trash_mbox")
      (comment "Move messages to this folder when deleting them
...
")
      ...
      (comment "Automatically create the above listed default folders on user login
")
      (value "False")
      (section "ui")
      (state "init")
      ...
      (comment "Protect the default folders from renames, deletes, and subscription changes
")
      (value "True")
      (section "ui")
      (state "init")
      ...
      (comment "Disable localization of the default folder names listed above
")
      (value "False")
      (section "ui")
      (state "init")
      ...
      (comment "If in your system 0 quota means no limit set this option to true
")
      (value "False")
      (section "ui")
      (state "init")
      ...
      (comment "Make use of the built-in spell checker. It is based on GoogieSpell.
")
      (value "True")
      (section "ui")
      (state "init")
      ...
      (comment "Set the spell checking engine. Possible values:
...
")
      ...
      (name "spellcheck_uri")
      (comment "For locally installed Nox Spell Server or After the Deadline services,
...
")
      ...
      (comment "These languages can be selected for spell checking.
...
")
      ...
      (comment "Makes that words with all letters capitalized will be ignored (e.g. GOOGLE)
")
      (value "False")
      (section "ui")
      (state "init")
      ...
      (comment "Makes that words with numbers will be ignored (e.g. g00gle)
")
      (value "False")
      (section "ui")
      (state "init")
      ...
      (comment "Makes that words with symbols will be ignored (e.g. g@@gle)
")
      (value "False")
      (section "ui")
      (state "init")
      ...
      (name "sig_max_lines")
      (comment "Number of lines at the end of a message considered to contain the signature.
...
")
      ...
      (name "max_pagesize")
      (comment "Don't let users set pagesize to more than this value if set
")
      (value "200")
      (section "ui")
      (state "init")
      ...
      (comment "Minimal value of user's 'refresh_interval' setting (in seconds)
")
      (value "60")
      (section "ui")
      (state "init")
      ...
      (name "undo_timeout")
      (comment "Specifies for how many seconds the Undo button will be available
...
")
      ...
      (comment "A static list of canned responses which are immutable for the user
")
      (array (list
          (list
            ...
            (text "Static Response One"))
          (list
            
            (name "Canned Response 2")
            
            (text "Static Response Two"))))
      (section "ui")
      (state "init")
      
      (name "keyservers")
      (comment "List of HKP key servers for PGP public key lookups in Enigma/Mailvelope
...
")
      ...
      (comment "This indicates which type of address book to use. Possible choices:
...
")
      ...
      (name "ldap_public")
      (comment "In order to enable public ldap search, configure an array like the Verisign
...
")
      ...
      (option (list
          "ldap_public"
          "Verisign"))
      (comment "If you are going to use LDAP for individual address books, you will need to
...
")
      (array (list
          ...
          (autovalues (list
              ...
              (uid "md5(microtime())")
              (mail "{givenname}.{sn}@mydomain.com")))
          (sort "cn")
          (scope "sub")
      ...
      (comment "An ordered array of the ids of the addressbooks that should be searched
...
")
      ...
      (comment "The minimum number of characters required to be typed in an autocomplete field
...
")
      ...
      (comment "Number of parallel autocomplete requests.
...
")
      ...
      (comment "Show address fields in this order
...
")
      ...
      (comment "Matching mode for addressbook search (including autocompletion)
...
")
      ...
      (comment "List of fields used on contacts list and for autocompletion searches
...
")
      ...
      (comment "Template of contact entry on the autocompletion list.
...
")
      ...
      (comment "Enables using standard browser windows (that can be handled as tabs)
...
")
      ...
      (comment "Sort contacts by this col (preferably either one of name, firstname, surname)
")
      (value "surname")
      (section "userprefs")
      (state "init")
      ...
      (comment "The way how contact names are displayed in the list.
...
")
      ...
      (name "timezone")
      (comment "Use this timezone to display date/time
...
")
      ...
      (name "show_images")
      (comment "Display remote resources (inline images, styles)
0 - Never, always ask
...
")
      ...
      (name "htmleditor")
      (comment "Compose html formatted messages by default
 0 - never,
 1 - always,
...
")
      ...
      (comment "Save copies of compose messages in the browser's local storage
...
")
      ...
      (name "layout")
      (comment "Interface layout. Default: 'widescreen'.
...
")
      ...
      (name "mail_read_time")
      (comment "Mark as read when viewing a message (delay in seconds)
...
")
      ...
      (comment "Encoding of long/non-ascii attachment names:
...
")
      ...
      (name "skip_deleted")
      (comment "Set true if deleted messages should not be displayed
...
")
      ...
      (comment "Set true to Mark deleted messages as read as well as deleted
...
")
      ...
      (comment "Set to true to never delete messages immediately
...
")
      ...
      (comment "Default interval for auto-refresh requests (in seconds)
...
")
      ...
      (name "display_next")
      (comment "If true, after message/contact delete/move, the next message/contact will be displayed
")
      (value "True")
      (section "userprefs")
      (state "init")
      ...
      (comment "Default messages listing mode. One of 'threads' or 'list'.
")
      (value "list")
      (section "userprefs")
      (state "init")
      ...
      (comment "0 - Do not expand threads
...
")
      ...
      (name "sig_below")
      (comment "By default the signature is placed depending on cursor position (reply_mode).
...
")
      ...
      (name "sig_separator")
      (comment "Enables adding of standard separator to the signature
")
      (value "True")
      (section "userprefs")
      (state "init")
      ...
      (name "force_7bit")
      (comment "Use MIME encoding (quoted-printable) for 8bit characters in message body
")
      (value "False")
      (section "userprefs")
      (state "init")
      ...
      (name "search_mods")
      (comment "Defaults of the search field configuration.
...
")
      ...
      (comment "Defaults of the addressbook search field configuration.
...
")
      ...
      (name "mdn_requests")
      (comment "Behavior if a received message requests a message delivery notification (read receipt)
...
")
      ...
      (name "dsn_default")
      (comment "Delivery Status Notification checkbox default state
")
      (value "0")
      (section "userprefs")
      (state "init")
      ...
      (comment "Place replies in the folder of the message being replied to
")
      (value "False")
      (section "userprefs")
      (state "init")
      ...
      (comment "Sets default mode of Forward feature to \"forward as attachment\"
")
      (value "False")
      (section "userprefs")
      (state "init")
      ...
      (comment "Defines address book (internal index) to which new contacts will be added
...
")
      ...
      (comment "Skip alternative email addresses in autocompletion (show one address per contact)
")
      (value "False")
      (section "userprefs")
      (state "init")
      ...
      (name "default_font")
      (comment "Default font for composed HTML message.
...
")
      ...
      (comment "Default font size for composed HTML message.
...
")
      ...
      (comment "Enables display of email address with name instead of a name (and address in title)
")
      (value "False")
      (section "userprefs")
      (state "init")
      ...
      (name "reply_all_mode")
      (comment "Default behavior of Reply-All button:
0 - Reply-All always
...
")
      ...
      (section "userprefs")
      (state "init")))
  (roundcube__default_configuration (list
      
      (name "db_dsnw")
      (value (jinja "{{ \"sqlite:///\" + roundcube__git_dest + \"/\"
      ...
      (name "db_dsnw")
      (value (jinja "{{ \"mysql://\" + roundcube__database_map[roundcube__database].dbuser | d() + \":\"
      ...
      (name "db_dsnw")
      (value (jinja "{{ \"pgsql://\" + roundcube__database_map[roundcube__database].dbuser | d() + \":\"
                          + roundcube__database_map[roundcube__database].dbpass | d() + \"@\"
                          + roundcube__database_map[roundcube__database].dbhost | d() + \"/\"
                          + roundcube__database_map[roundcube__database].dbname | d() }}"))
      (state (jinja "{{ \"present\"
      ...
      (comment "Log driver: \"syslog\", \"stdout\" or \"file\"
")
      (value (jinja "{{ roundcube__log_driver }}"))
      
      (name "imap_host")
      (value (jinja "{{ roundcube__imap_server }}"))
      
      (name "imap_cache")
      (value "memcached")
      (state (jinja "{{ \"present\" if roundcube__memcached_enabled | bool else \"ignore\" }}"))
      ...
      (name "smtp_host")
      (value (jinja "{{ roundcube__smtp_server }}"))
      
      (name "smtp_user")
      (value (jinja "{{ roundcube__smtp_user }}"))
      
      (name "smtp_pass")
      (value (jinja "{{ roundcube__smtp_pass }}"))
      ...
      (name "memcache_hosts")
      (value (jinja "{{ roundcube__memcached_hosts }}"))
      (state (jinja "{{ \"present\" if roundcube__memcached_enabled | bool else \"ignore\" }}"))
      
      (name "redis_hosts")
      (value (jinja "{{ roundcube__redis_hosts }}"))
      (state (jinja "{{ \"present\" if roundcube__redis_enabled | bool else \"ignore\" }}"))
      
      (name "skin_logo")
      (value (jinja "{{ roundcube__skin_logo }}"))
      
      (name "force_https")
      (value "True")
      
      (name "session_storage")
      (value (jinja "{{ roundcube__session_storage }}"))
      (state (jinja "{{ \"present\" if roundcube__session_storage | d() else \"ignore\" }}"))
      
      (name "des_key")
      (value (jinja "{{ roundcube__des_key }}"))
      
      (name "cipher_method")
      (value "AES-256-CBC")
      
      (name "username_domain")
      (value (jinja "{{ roundcube__username_domain }}"))
      
      (name "sendmail_delay")
      (value "5")
      
      (name "product_name")
      (value (jinja "{{ roundcube__product_name }}"))
      ...
      (name "plugins")
      (value (jinja "{{ roundcube__combined_plugins | debops.debops.parse_kv_items
      ...
      (name "autocomplete_addressbooks")
      (value (jinja "{{ ([\"sql\"] + ([\"People\"] if roundcube__ldap_enabled | bool else []))
               | flatten }}"))
      (state (jinja "{{ \"present\" if roundcube__ldap_enabled | bool else \"ignore\" }}"))
      ...
      (array (list
          ...
          (name (jinja "{{ roundcube__ldap_addressbook_name }}"))
          (hosts (jinja "{{ ([\"tls://\"] if roundcube__ldap_use_tls else [\"\"])
                   | product(roundcube__ldap_hosts) | map(\"join\")
                   | product([roundcube__ldap_port]) | map(\"join\") }}"))
          (ldap_version "3")
          (user_specific "True")
          (base_dn (jinja "{{ roundcube__ldap_people_dn | join(\",\") }}"))
          (bind_dn (jinja "{{ \"uid=%u,\" + roundcube__ldap_people_dn | join(\",\") }}"))
          (bind_pass "")
          (search_base_dn (jinja "{{ roundcube__ldap_base_dn | join(\",\") }}"))
          (search_filter "(& (objectClass=mailRecipient) (| (uid=%u) (mail=%fu) ) )")
          (search_bind_dn (jinja "{{ roundcube__ldap_binddn }}"))
          (search_bind_pw (jinja "{{ roundcube__ldap_bindpw }}"))
          (filter "(objectClass=inetOrgPerson)")
          (scope "sub")
          (searchonly "True")
          (vlv "False")
          ...
          (search_fields (list
              ...
              "mail"
              "telephoneNumber"))
          (hidden (jinja "{{ roundcube__ldap_hidden }}"))
          (writable "False")
          (groups (list
              
              (base_dn (jinja "{{ roundcube__ldap_groups_dn | join(\",\") }}"))
              (filter "(objectClass=groupOfNames)")
              (object_classes (list
                  "groupOfNames"))))
          (fieldmap (list
              (jinja "{{ roundcube__ldap_field_map }}")))))
      
      (name "default_charset")
      (value "UTF-8")
      
      (name "skin")
      (value (jinja "{{ roundcube__skin_folder }}"))
      ...
      (name "swipe_actions")
      (comment "Configuration for the 'swipe' Roundcube plugin
")
      (array (list
      ...
      (value "True")
      (state "present")))
  (roundcube__configuration (list))
  (roundcube__group_configuration (list))
  (roundcube__host_configuration (list))
  (roundcube__combined_configuration (jinja "{{ roundcube__original_configuration
                                       + roundcube__default_configuration
                                       + roundcube__configuration
                                       + roundcube__group_configuration
                                       + roundcube__host_configuration }}"))
  (roundcube__default_configuration_sections (list
      ...
      (name "unknown")
      (title "OTHER OPTIONS")))
  (roundcube__configuration_sections (list))
  (roundcube__combined_configuration_sections (jinja "{{ roundcube__default_configuration_sections
                                                + roundcube__configuration_sections }}"))
  (roundcube__default_plugins (list
...
")
          ...
          (comment "Zip selection of mail messages
...
")
          ...
          (comment "Charset to use for filenames inside the zip
")
          (value "UTF-8")))
      ...
      (name "contextmenu")
      (package "johndoh/contextmenu" (jinja "{{ \":3.2.1\"
      ...
      (options (list
          ...
          (comment "Automatically show the folder size without clicking on the toolbar button.
...
")
      ...
      (options (list
          ...
          (value (jinja "{{ \"https://cloud.\" + ansible_domain + \"/\" }}"))))
      
      (name "new_user_identity")
      (state (jinja "{{ \"enabled\" if roundcube__ldap_enabled | bool else \"ignore\" }}"))
      (options (list
          
          (name "new_user_identity_addressbook")
          (comment "The id of the address book to use to automatically set a
...
")
          ...
          (comment "When automatically setting a user's full name in their
new identity, match the user's login name against this field.
")
          (value "mail")
          
          (name "new_user_identity_onlogin")
          (comment "Determine whether to import user's identities on each login.
...
")
      ...
      (name "password")
      (state (jinja "{{ \"enabled\" if roundcube__ldap_password_enabled | bool else \"absent\" }}"))
      (options (list
          
          (name "password_driver")
          (comment "Password Plugin options
-----------------------
A driver to use for password change. Default: \"sql\".
See README file for list of supported driver names.
")
          (value (jinja "{{ \"ldap_exop\" if roundcube__ldap_password_enabled | bool else \"sql\" }}"))
          
          (name "password_strength_driver")
          (comment "A driver to use for checking password strength. Default: null (disabled).
See README file for list of supported driver names.
")
          (value null)
          
          (name "password_confirm_current")
          (comment "Determine whether current password is required to change password.
Default: false.
")
          (value "True")
          
          (name "password_minimum_length")
          (comment "Require the new password to be a certain length.
set to blank to allow passwords of any length
")
          (value "0")
          
          (name "password_minimum_score")
          (comment "Require the new password to have at least the specified strength score.
Note: Password strength is scored from 1 (week) to 5 (strong).
")
          (value "0")
          
          (name "password_log")
          (comment "Enables logging of password changes into logs/password
")
          (value "False")
          
          (name "password_login_exceptions")
          (comment "Comma-separated list of login exceptions for which password change
will be not available (no Password tab in Settings)
")
          (value null)
          
          (name "password_hosts")
          (comment "Array of hosts that support password changing.
...
")
          ...
          (comment "Enables saving the new password even if it matches the old password. Useful
for upgrading the stored passwords after the encryption scheme has changed.
")
          (value "False")
          
          (name "password_force_new_user")
          (comment "Enables forcing new users to change their password at their first login.
")
          (value "False")
          
          (name "password_algorithm")
          (comment "Default password hashing/crypting algorithm.
...
")
          ...
          (comment "Password prefix (e.g. {CRYPT}, {SHA}) for passwords generated
using password_algorithm above. Default: empty.
")
          (value "")
          
          (name "password_dovecotpw")
          (comment "Path for dovecotpw/doveadm-pw (if not in the $PATH).
Used for password_algorithm = 'dovecot'.
$config['password_dovecotpw'] = '/usr/local/sbin/dovecotpw'; // for dovecot-1.x
")
          (value "/usr/bin/doveadm pw")
          
          (name "password_dovecotpw_method")
          (comment "Dovecot password scheme.
Used for password_algorithm = 'dovecot'.
")
          (value "CRAM-MD5")
          
          (name "password_dovecotpw_with_method")
          (comment "Enables use of password with method prefix, e.g. {MD5}$1$LUiMYWqx$fEkg/ggr/L6Mb2X7be4i1/
when using password_algorithm=dovecot
")
          (value "False")
          
          (name "password_blowfish_cost")
          (comment "Iteration count parameter for Blowfish-based hashing algo.
...
")
          ...
          (comment "Number of rounds for the sha256 and sha512 crypt hashing algorithms.
...
")
          ...
          (comment "This option temporarily disables the password change functionality.
...
")
          ...
          (comment "Various drivers/setups use different format of the username.
...
")
          ...
          (name "password_query")
          (comment "The SQL query used to change the password.
...
")
          ...
          (comment "By default the crypt() function which is used to create the %c
...
")
          ...
          (comment "By default domains in variables are using unicode.
Enable this option to use punycoded names
")
          (value "False")
          
          (name "password_hash_algorithm")
          (comment "Using a password hash for %n and %q variables (deprecated, use %P).
...
")
          ...
          (comment "You can also decide whether the hash should be provided
as hex string or in base64 encoded format.
")
          (value "False")
          
          (name "password_pop_host")
          (comment "Poppassd Driver options
-----------------------
...
")
          ...
          (comment "TCP port used for poppassd connections (default: 106)
")
          (value "106")
          
          (name "password_saslpasswd_args")
          (comment "SASL Driver options
-------------------
...
")
          ...
          (comment "LDAP, LDAP_SIMPLE and LDAP_EXOP Driver options
-----------------------------------
...
Default: 'localhost'
")
          (value (jinja "{{ (roundcube__ldap_hosts | first)
                   if roundcube__ldap_hosts | d()
                   else \"\" }}"))
          
          (name "password_ldap_port")
          (comment "LDAP server port to connect to
Default: '389'
")
          (value (jinja "{{ roundcube__ldap_port }}"))
          
          (name "password_ldap_starttls")
          (comment "TLS is started after connecting
Using TLS for password modification is recommended.
Default: false
")
          (value (jinja "{{ roundcube__ldap_use_tls | bool }}"))
...
")
          ...
          (comment "LDAP base name (root directory)
Example: 'dc=example,dc=com'
")
          (value (jinja "{{ roundcube__ldap_base_dn | join(\",\") }}"))
          
          (name "password_ldap_method")
          (comment "LDAP connection method
...
")
          (value (jinja "{{ roundcube__ldap_binddn }}"))
          
          (name "password_ldap_searchPW")
          (comment "LDAP search password
...
")
          (value (jinja "{{ roundcube__ldap_bindpw }}"))
          
          (name "password_ldap_search_base")
          (comment "LDAP search base
...
")
          (value "(& (objectClass=mailRecipient) (| (uid=%name) (mail=%login) ) )")
          
          (name "password_ldap_encodage")
          (comment "LDAP password hash type
...
")
          ...
          (comment "LDAP password attribute
...
")
          ...
          (comment "LDAP password force replace
...
")
          ...
          (comment "LDAP Password Last Change Date
...
")
          ...
          (comment "LDAP Samba password attribute, e.g. sambaNTPassword
Name of the LDAP's Samba attribute used for storing user password
")
          (value "")
          
          (name "password_ldap_samba_lchattr")
          (comment "LDAP Samba Password Last Change Date attribute, e.g. sambaPwdLastSet
Some places use an attribute to store the date of the last password change
...
")
          ...
          (comment "LDAP PPolicy Driver options
-----------------------------------
...
")
          ...
          (comment "CA Certificate file if in URI is LDAPS connection
")
          (value "/etc/ssl/certs/ca-certificates.crt")
          
          (name "password_directadmin_host")
          (comment "DirectAdmin Driver options
--------------------------
...
")
          ...
          (comment "TCP port used for DirectAdmin connections
")
          (value "2222")
          
          (name "password_vpopmaild_host")
          (comment "vpopmaild Driver options
-----------------------
The host which changes the password
")
          (value "localhost")
          ...
          (comment "TCP port used for vpopmaild connections
")
          (value "89")
          
          (name "password_vpopmaild_timeout")
          (comment "Timeout used for the connection to vpopmaild (in seconds)
")
          (value "10")
          
          (name "password_cpanel_host")
          (comment "cPanel Driver options
--------------------------
...
")
          ...
          (comment "The cPanel admin username
")
          (value "username")
          
          (name "password_cpanel_password")
          (comment "The cPanel admin password
")
          (value "password")
          
          (name "password_cpanel_hash")
          (comment "The cPanel admin hash
...
")
          ...
          (comment "The cPanel port to use
")
          (value "2087")
          
          (name "password_cpanel_webmail_host")
          (comment "cPanel Webmail Driver options
-----------------------------
The cPanel Host name
")
          (value "host.domain.com")
...
")
          ...
          (comment "XIMSS (Communigate server) Driver options
-----------------------------------------
...
")
          ...
          (comment "XIMSS port on Communigate server
")
          (value "11024")
          
          (name "password_chpasswd_cmd")
          (comment "chpasswd Driver options
---------------------
...
")
          ...
          (comment "pw_usermod Driver options
--------------------------
...
")
          ...
          (comment "Location of expect script (see helpers/passwd-expect)
")
          (value "")
          
          (name "password_expect_params")
          (comment "Arguments for the expect script. See the helpers/passwd-expect file for details.
...
")
          ...
          (comment "Location of smbpasswd binary (default: /usr/bin/smbpasswd)
")
          (value "/usr/bin/smbpasswd")
          
          (name "password_gearman_host")
          (comment "gearman driver options
---------------------
...
")
          ...
          (comment "Plesk/PPA Driver options
--------------------
...
")
          ...
          (comment "Modoboa Driver options
---------------------
...
")
      ...
      (options (list
          ...
          (name "help_source")
          (comment "Help content iframe source
...
")
          ...
          (name "help_index_map")
          (comment "Map task/action combinations to deep-links
...
")
          ...
          (comment "Map to translate Roundcube language codes into help document languages
...
")
          ...
          (name "help_about_url")
          (comment "Enter an absolute URL to a page displaying information about this webmail
...
")
          ...
          (comment "Add flag to messages marked as spam (flag will be removed when marking as ham)
If you do not want to use message flags set this to false
")
          (value "Junk")
          
          (name "markasjunk_ham_flag")
          (comment "Add flag to messages marked as ham (flag will be removed when marking as spam)
If you do not want to use message flags set this to false
")
          (value "NonJunk")
          ...
          (comment "The mark as spam/ham icon can either be displayed on the toolbar or as part of the mark messages menu.
Set to False to use Mark menu instead of the toolbar. Default: true.
")
          (value "True")
          
          (name "markasjunk_move_spam")
          (comment "Learn any message moved to the spam mailbox as spam (not just when the button is pressed)
")
          (value "True")
          
          (name "markasjunk_move_ham")
          (comment "Learn any message moved from the spam mailbox to the ham mailbox as ham (not just when the button is pressed)
")
          (value "True")
          
          (name "markasjunk_permanently_remove")
          (comment "Some drivers create new copies of the target message(s), in this case the original message(s) will be deleted
...
")
          ...
          (comment "Activate markasjunk for selected mail hosts only. If this is not set all mail hosts are allowed.
Example: $config['markasjunk_allowed_hosts'] = array('mail1.domain.tld', 'mail2.domain.tld');
")
          (value null)
          
          (name "markasjunk_host_config")
          (comment "Load specific config for different mail hosts
...
")
          ...
          (comment "cmd_learn Driver options
------------------------
...
")
          ...
          (comment "The command used to learn that a message is ham
The command can contain the following macros that will be expanded as follows:
     %u is replaced with the username (from the session info)
     %l is replaced with the local part of the username (if the username is an email address)
     %d is replaced with the domain part of the username (if the username is an email address or default mail domain if not)
...
")
          ...
          (comment "dir_learn Driver options
------------------------
...
")
          ...
          (comment "The full path of the directory used to store ham (must be writable by webserver)
")
          (value null)
...
")
          ...
          (comment "email_learn Driver options
--------------------------
...
")
          ...
          (comment "The email address that ham messages will be sent to
The address can contain the following macros that will be expanded as follows:
     %u is replaced with the username (from the session info)
     %l is replaced with the local part of the username (if the username is an email address)
     %d is replaced with the domain part of the username (if the username is an email address or default mail domain if not)
...
")
          ...
          (comment "The email subject (when sending as attachment)
...
")
          ...
          (comment "sa_blacklist Driver options
---------------------------
...
")
          ...
          (comment "amavis_blacklist Driver options
---------------------------
...
")
          ...
          (comment "edit_headers Driver options
---------------------------
...
")
          ...
          (comment "Patterns to match and replace headers for spam messages
Replacement method uses preg_replace - http://www.php.net/manual/function.preg-replace.php
WARNING: Be sure to match the entire header line, including the name of the header, also use ^ and $ and the 'm' flag
see the README for an example
TEST CAREFULLY BEFORE USE ON REAL MESSAGES
")
      ...
      (options (list
          ...
          (comment "managesieve server address, default is localhost.
...
")
          (value (jinja "{{ roundcube__sieve_server }}"))
          
          (name "managesieve_auth_type")
          (comment "Authentication method. Can be CRAM-MD5, DIGEST-MD5, PLAIN, LOGIN, EXTERNAL
or none. Optional, defaults to best method supported by server.
")
          (value null)
          
          (name "managesieve_auth_cid")
          (comment "Optional managesieve authentication identifier to be used as authorization proxy.
...
")
          ...
          (comment "Optional managesieve authentication password to be used for imap_auth_cid
")
          (value null)
          
          (name "managesieve_conn_options")
          (comment "Connection scket context options
See https://php.net/manual/en/context.ssl.php
The example below enables server certificate validation
")
          (array (list
          ...
          (comment "A file with default script content (eg. spam filter)
")
          (value "/etc/dovecot/sieve/global")
          
          (name "managesieve_script_name")
          (comment "The name of the script which will be used when there's no user script
")
          (value "managesieve")
          
          (name "managesieve_mbox_encoding")
          (comment "Sieve RFC says that we should use UTF-8 encoding for mailbox names,
...
")
          ...
          (comment "I need this because my dovecot (with listescape plugin) uses
':' delimiter, but creates folders with dot delimiter
")
          (value "")
          
          (name "managesieve_disabled_extensions")
          (comment "Disabled sieve extensions (body, copy, date, editheader, encoded-character,
...
")
          ...
          (comment "Enables debugging of conversation with sieve server. Logs it into <log_dir>/sieve
")
          (value "False")
          
          (name "managesieve_kolab_master")
          (comment "Enables features described in http://wiki.kolab.org/KEP:14
")
          (value "False")
          
          (name "managesieve_filename_extension")
          (comment "Script name extension used for scripts including. Dovecot uses '.sieve',
Cyrus uses '.siv'. Doesn't matter if you have managesieve_kolab_master disabled.
")
          (value ".sieve")
          
          (name "managesieve_filename_exceptions")
          (comment "List of reserved script names (without extension).
Scripts listed here will be not presented to the user.
")
          (value (list))
          
          (name "managesieve_domains")
          (comment "List of domains limiting destination emails in redirect action
If not empty, user will need to select domain from a list
")
          (value (list))
          
          (name "managesieve_default_headers")
          (comment "Default list of entries in header selector
")
          (value (list
              "Subject"
              "From"
          ...
          (comment "Enables separate management interface for vacation responses (out-of-office)
...
")
          ...
          (comment "Enables separate management interface for setting forwards (redirect to and copy to)
0 - no separate section (default),
...
")
          ...
          (comment "Default vacation interval (in days).
...
")
          ...
          (comment "Some servers require vacation :addresses to be filled with all
...
")
          ...
          (comment "Sometimes you want to always reply with mail email address
...
")
          ...
          (comment "Supported methods of notify extension. Default: 'mailto'
")
          (value (list
              "mailto"))
...
")
          ...
          (comment "List of hosts that support managesieve.
...
")
          (value null)))))
  (roundcube__plugins (list))
  (roundcube__group_plugins (list))
  (roundcube__host_plugins (list))
  (roundcube__combined_plugins (jinja "{{ roundcube__default_plugins
                                 + roundcube__plugins
  ...
  (roundcube__keyring__dependent_gpg_user (jinja "{{ roundcube__user }}"))
  (roundcube__keyring__dependent_gpg_keys (list
      
      (user (jinja "{{ roundcube__user }}"))
      (group (jinja "{{ roundcube__group }}"))
      (home (jinja "{{ roundcube__home }}"))
      (id (jinja "{{ roundcube__git_gpg_key }}"))
      (jinja "{{ roundcube__git_additional_gpg_keys }}")))
  (roundcube__ldap__dependent_tasks (list
      
      (name "Create roundcube account for " (jinja "{{ roundcube__ldap_device_dn | join(\",\") }}"))
      (dn (jinja "{{ roundcube__ldap_binddn }}"))
      (objectClass (jinja "{{ roundcube__ldap_self_object_classes }}"))
      (attributes (jinja "{{ roundcube__ldap_self_attributes }}"))
      (no_log (jinja "{{ debops__no_log | d(True) }}"))
      (state (jinja "{{ \"present\" if roundcube__ldap_password_enabled and
                            roundcube__ldap_device_dn | d() else \"ignore\" }}"))))
  (roundcube__nginx__dependent_servers (list
      
      (name (jinja "{{ roundcube__fqdn }}"))
      (filename "debops.roundcube")
      (by_role "debops.roundcube")
      (type "php")
      (root (jinja "{{ roundcube__git_dest }}"))
      (webroot_create "False")
      (access_policy (jinja "{{ roundcube__nginx_access_policy }}"))
      (index "index.php")
      (options "autoindex off;
client_max_body_size " (jinja "{{ roundcube__max_file_size }}") "M;
client_body_buffer_size 128k;
")
      (location_list (list
          
          (pattern "/")
          (options "try_files $uri $uri/ @roundcube;
")
          
          (pattern "@roundcube")
          (options "rewrite ^/favicon\\.ico$ skins/" (jinja "{{ roundcube__skin_folder }}") "/images/favicon.ico last;
")
...
")
  ...
  (roundcube__nginx__dependent_upstreams (list
      ...
      (type "php")
      (php_pool "roundcube")))
  (roundcube__php__dependent_packages (list
      (jinja "{{ roundcube__base_php_packages }}")
      (jinja "{{ roundcube__optional_php_packages }}")
      (jinja "{{ roundcube__custom_php_packages }}")))
  (roundcube__php__dependent_pools 
    ...
    (home (jinja "{{ roundcube__home }}"))
    (php_values 
      (upload_max_filesize (jinja "{{ roundcube__max_file_size }}") "M")
      (post_max_size (jinja "{{ roundcube__max_file_size }}") "M")
      (file_uploads "on")
  ...
  (roundcube__nodejs__npm_dependent_packages (list
      "less"
      "tslib")))
