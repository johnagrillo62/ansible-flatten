(playbook "debops/ansible/roles/libvirtd/defaults/main.yml"
  (libvirtd__kvm_support (jinja "{{ True
                           if (ansible_virtualization_type == \"kvm\" and
                               (ansible_virtualization_role == \"host\" or
                                libvirtd__register_hw_virt.stdout | d()))
                           else False }}"))
  (libvirtd__base_packages (list
  ...
  (libvirtd__base_packages_map 
    ...
    (xenial (list
        "libvirt-bin")))
  (libvirtd__kvm_packages (jinja "{{ [\"qemu-system-x86\", \"qemu-utils\"]
                            + ([\"qemu-kvm\"]
                              if ansible_distribution_release in [\"stretch\",
                                   \"buster\", \"bionic\", \"focal\"]
                              else []) }}"))
  (libvirtd__network_packages (list
      "dnsmasq-base"
      "bridge-utils"
      "ebtables"))
  (libvirtd__misc_packages (list
      "gawk"
      "netcat-openbsd"
      "ovmf"
      "pm-utils"
  ...
  (libvirtd__packages (list))
  (libvirtd__version (jinja "{{ ansible_local.libvirtd.version | d(\"0.0.0\") }}"))
  (libvirtd__deployment_mode (jinja "{{ \"opennebula\"
                               if (\"debops_service_opennebula_node\" in group_names)
                               else \"libvirt\" }}"))
  (libvirtd__admins (jinja "{{ [ansible_user | d(lookup(\"env\", \"USER\"))]
                      if ansible_user | d(lookup(\"env\", \"USER\")) != \"root\"
                      else [] }}"))
  (libvirtd__unix_sock_group (jinja "{{ ansible_local.libvirtd.unix_sock_group | d(\"libvirt\") }}"))
  (libvirtd__system_uri "qemu:///system")
  (libvirtd__uri_aliases 
    (localhost (jinja "{{ libvirtd__system_uri }}")))
  (libvirtd__default_uri "localhost")
  (libvirtd__original_configuration (list
...
")
      ...
      (name "mdns_adv")
      (comment "Flag toggling mDNS advertisement of the libvirt service.
...
")
      ...
      (name "mdns_name")
      (comment "Override the default mDNS advertisement name. This must be
...
")
      ...
      (comment "Set the UNIX socket permissions for the admin interface socket.
...
")
      ...
      (name "unix_sock_dir")
      (comment "Set the name of the directory in which sockets will be found/created.
")
...
")
      ...
      (name "access_drivers")
      (comment "Change the API access control scheme
...
")
      ...
      (comment "Flag to disable verification of our own server certificates
...
")
      ...
      (comment "A whitelist of allowed SASL usernames. The format for username
...
")
      ...
      (name "tls_priority")
      (comment "Override the compile time default TLS priority string. The
...
")
      ...
      (comment "The maximum length of queue of connections waiting to be
...
")
      ...
      (comment "The maximum length of queue of accepted but not yet
...
")
      ...
      (name "prio_workers")
      (comment "The number of priority workers. If all workers from above
...
")
      ...
      (comment "Same processing controls, but this time for the admin interface.
...
")
      ...
      (name "log_level")
      (comment "Logging level: 4 errors, 3 warnings, 2 information, 1 debug
...
")
      ...
      (name "audit_level")
      (comment "This setting allows usage of the auditing subsystem to be altered:
...
")
      ...
      (name "audit_logging")
      (comment "If set to 1, then audit messages will also be sent
...
")
      ...
      (name "host_uuid")
      (comment "Host UUID is read from one of the sources specified in host_uuid_source.
...
")
      ...
      (comment "This allows libvirtd to detect broken client connections or even
...
")
      ...
      (comment "These configuration options are no longer used.  There is no way to
...
")
      ...
      (comment "Keepalive settings for the admin interface
")
      (value "5")
      (section "keepalive")
      (state "comment")
      ...
      (state "comment")
      (weight "6")))
  (libvirtd__default_configuration (list
      
      (unix_sock_group (jinja "{{ libvirtd__unix_sock_group }}"))
  ...
  (libvirtd__configuration (list))
  (libvirtd__group_configuration (list))
  (libvirtd__host_configuration (list))
  (libvirtd__combined_configuration (jinja "{{ libvirtd__original_configuration
                                      + libvirtd__default_configuration
                                      + libvirtd__configuration
                                      + libvirtd__group_configuration
                                      + libvirtd__host_configuration }}"))
  (libvirtd__configuration_sections (list
...
")
      ...
      (state "absent")))
  (libvirtd__python__dependent_packages3 (list
      "python3-libvirt"))
  (libvirtd__python__dependent_packages2 (list
      "python-libvirt"))
  (libvirtd__nsswitch__dependent_services (list
      "libvirt"
      "libvirt_guest")))
