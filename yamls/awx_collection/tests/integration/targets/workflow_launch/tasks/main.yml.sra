(playbook "awx_collection/tests/integration/targets/workflow_launch/tasks/main.yml"
  (tasks
    (task "Generate a random string for test"
      (ansible.builtin.set_fact 
        (test_id (jinja "{{ lookup('password', '/dev/null chars=ascii_letters length=16') }}")))
      (when "test_id is not defined"))
    (task "Generate names"
      (ansible.builtin.set_fact 
        (wfjt_name1 "AWX-Collection-tests-workflow_launch--wfjt1-" (jinja "{{ test_id }}"))
        (wfjt_name2 "AWX-Collection-tests-workflow_launch--wfjt1-" (jinja "{{ test_id }}") "-2")
        (approval_node_name "AWX-Collection-tests-workflow_launch_approval_node-" (jinja "{{ test_id }}"))))
    (task "Create workflows"
      (block (list
          
          (name "Create our workflow")
          (workflow_job_template 
            (name (jinja "{{ wfjt_name1 }}"))
            (state "present"))
          
          (name "Add a node")
          (workflow_job_template_node 
            (workflow_job_template (jinja "{{ wfjt_name1 }}"))
            (unified_job_template "Demo Job Template")
            (identifier "leaf"))
          (register "new_node")
          
          (name "Connect to controller server but request an invalid workflow")
          (workflow_launch 
            (workflow_template "Does Not Exist"))
          (ignore_errors "true")
          (register "result")
          
          (name "Assert that workflow launch failed with expected error")
          (ansible.builtin.assert 
            (that (list
                "result.failed | default(false)"
                "'Unable to find workflow job template' in result.msg")))
          
          (name "Run the workflow without waiting (this should just give us back a job ID)")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name1 }}"))
            (wait "false"))
          (ignore_errors "true")
          (register "result")
          
          (name "Assert result not failed")
          (ansible.builtin.assert 
            (that (list
                "result is not failed"
                "'id' in result['job_info']")))
          
          (name "Kick off a workflow and wait for it, but only for a second")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name1 }}"))
            (timeout "1"))
          (ignore_errors "true")
          (register "result")
          
          (name "Assert result failed")
          (ansible.builtin.assert 
            (that (list
                "result.failed | default(false)"
                "'Monitoring of Workflow Job - '~ wfjt_name1 ~ ' aborted due to timeout' in result.msg")))
          
          (name "Kick off a workflow and wait for it")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name1 }}")))
          (ignore_errors "true")
          (register "result")
          
          (name "Assert result did not fail")
          (ansible.builtin.assert 
            (that (list
                "not (result.failed | default(false))"
                "'id' in result['job_info']")))
          
          (name "Kick off a workflow with extra_vars but not enabled")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name1 }}"))
            (extra_vars 
              (var1 "My First Variable")
              (var2 "My Second Variable")))
          (ignore_errors "true")
          (register "result")
          
          (name "Assert result failed")
          (ansible.builtin.assert 
            (that (list
                "result.failed | default(false)"
                "'The field extra_vars was specified but the workflow job template does not allow for it to be overridden' in result.errors")))
          
          (name "Prompt the workflow's with survey")
          (workflow_job_template 
            (name (jinja "{{ wfjt_name1 }}"))
            (state "present")
            (survey_enabled "true")
            (ask_variables_on_launch "false")
            (survey 
              (name "")
              (description "")
              (spec (list
                  
                  (question_name "Basic Name")
                  (question_description "Name")
                  (required "true")
                  (type "text")
                  (variable "basic_name")
                  (min "0")
                  (max "1024")
                  (default "")
                  (choices "")
                  (new_question "true")
                  
                  (question_name "Choose yes or no?")
                  (question_description "Choosing yes or no.")
                  (required "false")
                  (type "multiplechoice")
                  (variable "option_true_false")
                  (min null)
                  (max null)
                  (default "yes")
                  (choices "yes
no")
                  (new_question "true")))))
          
          (name "Kick off a workflow with survey")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name1 }}"))
            (extra_vars 
              (basic_name "My First Variable")
              (option_true_false "no")))
          (ignore_errors "true")
          (register "result")
          
          (name "Assert result did not fail")
          (ansible.builtin.assert 
            (that (list
                "not (result.failed | default(false))")))
          
          (name "Prompt the workflow's extra_vars on launch")
          (workflow_job_template 
            (name (jinja "{{ wfjt_name1 }}"))
            (state "present")
            (ask_variables_on_launch "true"))
          
          (name "Kick off a workflow with extra_vars")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name1 }}"))
            (extra_vars 
              (basic_name "My First Variable")
              (var1 "My First Variable")
              (var2 "My Second Variable")))
          (ignore_errors "true")
          (register "result")
          
          (name "Assert did not fail")
          (ansible.builtin.assert 
            (that (list
                "not (result.failed | default(false))")))
          
          (name "Test waiting for an approval node that doesn't exit on the last workflow for failure.")
          (workflow_approval 
            (workflow_job_id (jinja "{{ result.id }}"))
            (name "Test workflow approval")
            (interval "1")
            (timeout "2")
            (action "deny"))
          (register "result")
          (ignore_errors "true")
          
          (name "Assert result failed")
          (ansible.builtin.assert 
            (that (list
                "result.failed | default(false)"
                "'Monitoring of Workflow Approval - Test workflow approval aborted due to timeout' in result.msg")))
          
          (name "Create new Workflow")
          (workflow_job_template 
            (name (jinja "{{ wfjt_name2 }}"))
            (state "present"))
          
          (name "Add a job node")
          (workflow_job_template_node 
            (workflow_job_template (jinja "{{ wfjt_name2 }}"))
            (unified_job_template "Demo Job Template")
            (identifier "leaf"))
          
          (name "Create approval node")
          (workflow_job_template_node 
            (identifier "approval_test")
            (approval_node 
              (name (jinja "{{ approval_node_name }}"))
              (timeout "900"))
            (workflow (jinja "{{ wfjt_name2 }}")))
          
          (name "Create link for approval node")
          (workflow_job_template_node 
            (identifier "approval_test")
            (workflow (jinja "{{ wfjt_name2 }}"))
            (always_nodes (list
                "leaf")))
          
          (name "Run the workflow without waiting This should pause waiting for approval")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name2 }}"))
            (wait "false"))
          (ignore_errors "true")
          (register "wfjt_info")
          
          (name "Wait for Job node wait to fail as it is waiting on approval")
          (awx.awx.workflow_node_wait 
            (workflow_job_id (jinja "{{ wfjt_info.id }}"))
            (name "Demo Job Template")
            (interval "1")
            (timeout "5"))
          (register "result")
          (ignore_errors "true")
          
          (name "Assert result didn't fail")
          (ansible.builtin.assert 
            (that (list
                "result.failed | default(false)"
                "'Monitoring of Workflow Node - Demo Job Template aborted due to timeout' in result.msg")))
          
          (name "Wait for approval node to activate and approve")
          (awx.awx.workflow_approval 
            (workflow_job_id (jinja "{{ wfjt_info.id }}"))
            (name (jinja "{{ approval_node_name }}"))
            (interval "1")
            (timeout "10")
            (action "deny"))
          (register "result")
          
          (name "Assert did not fail")
          (ansible.builtin.assert 
            (that (list
                "not (result.failed | default(false))"
                "result.changed | default(false)")))
          
          (name "Wait for workflow job to finish max 120s")
          (job_wait 
            (job_id (jinja "{{ wfjt_info.id }}"))
            (timeout "120")
            (job_type "workflow_jobs"))))
      (always (list
          
          (name "Clean up test workflow")
          (workflow_job_template 
            (name (jinja "{{ item }}"))
            (state "absent"))
          (with_items (list
              (jinja "{{ wfjt_name1 }}")
              (jinja "{{ wfjt_name2 }}"))))))))
