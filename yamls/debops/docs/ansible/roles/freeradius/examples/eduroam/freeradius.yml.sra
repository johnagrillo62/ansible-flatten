(playbook "debops/docs/ansible/roles/freeradius/examples/eduroam/freeradius.yml"
  (radius_domain (jinja "{{ ansible_domain }}"))
...
")
  ...
  tls-config tls-common {
    ...
    certificate_file = " (jinja "{{ radius_cert_file }}") "

    # The private key for the public certificate
    private_key_file = " (jinja "{{ radius_key_file }}") "
...
")
      ...
      (raw "linelog linelog_recv_request {
  filename = syslog
...
")
      ...
      (name "proxy.conf")
      (raw "home_server eduroam_flr_server_1 {
        ipaddr = 127.0.0.1
        secret = secret
...
client wireless_access_points_mgmt {
  ipaddr = " (jinja "{{ radius_access_point_subnet }}") "

  # This should be long and random
  secret = " (jinja "{{ radius_access_point_password }}") "
}
")
      (state "present")
      (no_log (jinja "{{ debops__no_log | d(True) }}"))
...
")
      ...
      (comment "This sets the same password for any user that tries to authenticate, do
not use in production environment
")
      (raw "DEFAULT Cleartext-Password := '" (jinja "{{ radius_test_user_password }}") "'
")
      (state "present")
      (no_log (jinja "{{ debops__no_log | d(True) }}")))))
