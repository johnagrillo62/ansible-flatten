(playbook "awx_collection/tests/integration/targets/job_wait/tasks/main.yml"
  (tasks
    (task "Generate a test ID"
      (set_fact 
        (test_id (jinja "{{ lookup('password', '/dev/null chars=ascii_letters length=16') }}")))
      (when "test_id is not defined"))
    (task "Generate random string for template and project"
      (set_fact 
        (jt_name "AWX-Collection-tests-job_wait-long_running-" (jinja "{{ test_id }}"))
        (proj_name "AWX-Collection-tests-job_wait-long_running-" (jinja "{{ test_id }}"))))
    (task
      (block (list
          
          (name "Create a project")
          (project 
            (name (jinja "{{ proj_name }}"))
            (scm_type "git")
            (scm_url "https://github.com/ansible/test-playbooks.git")
            (scm_update_on_launch "true")
            (organization "Default"))
          
          (name "Create a job template")
          (job_template 
            (name (jinja "{{ jt_name }}"))
            (playbook "sleep.yml")
            (job_type "run")
            (project (jinja "{{ proj_name }}"))
            (inventory "Demo Inventory")
            (extra_vars 
              (sleep_interval "600")))
          
          (name "Check module fails with correct msg")
          (job_wait 
            (job_id "99999999"))
          (register "result")
          (ignore_errors "yes")
          
          (assert 
            (that (list
                "result is failed"
                "result.msg == 'Unable to wait, no job_id 99999999 found: The requested object could not be found.' or result.msg == 'Unable to wait on job 99999999; that ID does not exist.'")))
          
          (name "Launch Demo Job Template (take happy path)")
          (job_launch 
            (job_template "Demo Job Template"))
          (register "job")
          
          (assert 
            (that (list
                "job is changed")))
          
          (name "Wait for the Job to finish")
          (job_wait 
            (job_id (jinja "{{ job.id }}")))
          (register "wait_results")
          
          (assert 
            (that (list
                "wait_results is successful"
                "'elapsed' in wait_results"
                "'id' in wait_results")))
          
          (name "Launch a long running job")
          (job_launch 
            (job_template (jinja "{{ jt_name }}")))
          (register "job")
          
          (assert 
            (that (list
                "job is changed")))
          
          (name "Timeout waiting for the job to complete")
          (job_wait 
            (job_id (jinja "{{ job.id }}"))
            (timeout "5"))
          (ignore_errors "yes")
          (register "wait_results")
          
          (assert 
            (that (list
                "'aborted due to timeout' in wait_results.msg"
                "'id' in wait_results")))
          
          (name "Async cancel the long running job")
          (job_cancel 
            (job_id (jinja "{{ job.id }}")))
          (async "3600")
          (poll "0")
          
          (name "Wait for the job to exit on cancel")
          (job_wait 
            (job_id (jinja "{{ job.id }}"))
            (timeout "60"))
          (register "wait_results")
          (ignore_errors "yes")
          
          (assert 
            (that (list
                "wait_results is failed"
                "wait_results.status == \"canceled\""
                "'Unable to find job with id' not in result.msg")))
          
          (name "Generate a random string for test")
          (set_fact 
            (test_id1 (jinja "{{ lookup('password', '/dev/null chars=ascii_letters length=16') }}")))
          (when "test_id1 is not defined")
          
          (name "Generate names")
          (set_fact 
            (wfjt_name2 "AWX-Collection-tests-workflow_launch--wfjt1-" (jinja "{{ test_id1 }}")))
          
          (name "Create our workflow")
          (workflow_job_template 
            (name (jinja "{{ wfjt_name2 }}"))
            (state "present"))
          
          (name "Add a node")
          (workflow_job_template_node 
            (workflow_job_template (jinja "{{ wfjt_name2 }}"))
            (unified_job_template "Demo Job Template")
            (identifier "leaf"))
          (register "new_node")
          
          (name "Kick off a workflow")
          (workflow_launch 
            (workflow_template (jinja "{{ wfjt_name2 }}")))
          (ignore_errors "yes")
          (register "workflow")
          
          (name "Wait for the Workflow Job to finish")
          (job_wait 
            (job_id (jinja "{{ workflow.job_info.id }}"))
            (job_type "workflow_jobs"))
          (register "wait_workflow_results")
          
          (assert 
            (that (list
                "wait_workflow_results is successful"
                "'elapsed' in wait_workflow_results"
                "'id' in wait_workflow_results")))))
      (always (list
          
          (name "Clean up test workflow")
          (workflow_job_template 
            (name (jinja "{{ wfjt_name2 }}"))
            (state "absent"))
          
          (name "Get all jobs for the template")
          (awx.awx.job_list 
            (query 
              (job_template (jinja "{{ jt_name }}"))))
          (register "job_list")
          
          (name "Delete the job template")
          (job_template 
            (name (jinja "{{ jt_name }}"))
            (playbook "sleep.yml")
            (job_type "run")
            (project (jinja "{{ proj_name }}"))
            (inventory "Demo Inventory")
            (state "absent"))
          (register "del_res")
          (until "del_res is succeeded")
          (retries "5")
          (delay "3")
          
          (name "Delete the project")
          (project 
            (name (jinja "{{ proj_name }}"))
            (organization "Default")
            (state "absent"))
          (register "del_res")
          (until "del_res is succeeded")
          (retries "5")
          (delay "3"))))))
