(playbook "kubespray/roles/network_plugin/calico/tasks/check.yml"
  (tasks
    (task "Stop if legacy encapsulation variables are detected (ipip)"
      (assert 
        (that (list
            "ipip is not defined"))
    ...
    (task "Stop if legacy encapsulation variables are detected (ipip_mode)"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Stop if legacy encapsulation variables are detected (calcio_ipam_autoallocateblocks)"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Stop if supported Calico versions"
      (assert 
        (that (list
            "calico_version in calico_crds_checksums.no_arch.keys()"))
        (msg "Calico version not supported " (jinja "{{ calico_version }}") " not in " (jinja "{{ calico_crds_checksums.no_arch.keys() }}")))
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check if calicoctl.sh exists"
      (stat 
        (path (jinja "{{ bin_dir }}") "/calicoctl.sh"))
      (register "calicoctl_sh_exists")
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check if calico ready"
      (command (jinja "{{ bin_dir }}") "/calicoctl.sh get ClusterInformation default")
    ...
    (task "Check that current calico version is enough for upgrade"
      (block (list
          
          (name "Get current calico version")
          (shell "set -o pipefail && " (jinja "{{ bin_dir }}") "/calicoctl.sh version | grep 'Client Version:' | awk '{ print $3}'")
          (args 
            (executable "/bin/bash"))
          ...
          (assert 
            ...
            (msg "Your version of calico is not fresh enough for upgrade. Minimum version is " (jinja "{{ calico_min_version_required }}") " supported by the previous kubespray release. But current version is " (jinja "{{ calico_version_on_server.stdout }}") ".
"))))
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}"))
      (when "calicoctl_sh_exists.stat.exists and calico_ready.rc == 0"))
    (task "Check that cluster_id is set and a valid IPv4 address if calico_rr enabled"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check that calico_rr nodes are in k8s_cluster group"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check vars defined correctly"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check calico network backend defined correctly"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check ipip and vxlan mode defined correctly"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check ipip and vxlan mode if simultaneously enabled"
      (assert 
        (that (list
    ...
    (task "Check ipip and vxlan mode if simultaneously enabled"
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Get Calico " (jinja "{{ calico_pool_name }}") " configuration"
      ...
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Set calico_pool_conf"
      (set_fact 
        (calico_pool_conf (jinja "{{ calico.stdout | from_json }}")))
      (when (list
          "ipv4_stack | bool"
          "calico is defined"
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check if inventory match current cluster configuration"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Get Calico " (jinja "{{ calico_pool_name }}") "-ipv6 configuration"
      ...
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Set calico_pool_ipv6_conf"
      (set_fact 
        (calico_pool_conf (jinja "{{ calico_ipv6.stdout | from_json }}")))
      (when (list
          "ipv6_stack | bool"
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check if ipv6 inventory match current cluster configuration"
      (assert 
        (that (list
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check kdd calico_datastore if calico_apiserver_enabled"
      (assert 
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check kdd calico_datastore if typha_enabled"
      (assert 
        (that "calico_datastore == \"kdd\"")
      ...
      (run_once "true")
      (delegate_to (jinja "{{ groups['kube_control_plane'][0] }}")))
    (task "Check ipip mode is Never for calico ipv6"
      (assert 
        (that (list
