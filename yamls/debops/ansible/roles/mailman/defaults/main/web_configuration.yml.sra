(playbook "debops/ansible/roles/mailman/defaults/main/web_configuration.yml"
  (mailman__web_original_configuration (list
      ...
      (name "allowed_hosts")
      (comment "Hosts/domain names that are valid for this site; required if DEBUG is False
...
")
      (state (jinja "{{ \"present\" if (mailman__database_type == \"sqlite\") else \"ignore\" }}"))
      (config 
        (default 
...
https://docs.djangoproject.com/en/1.8/ref/settings/#databases
")
      (state (jinja "{{ \"present\" if (mailman__database_type == \"mysql\") else \"ignore\" }}"))
      (config 
        (default 
      ...
      (comment "If you're behind a proxy, use the X-Forwarded-Host header
...
")
      ...
      (comment "And if your proxy does your SSL encoding for you, set SECURE_PROXY_SSL_HEADER
...
")
      ...
      (comment "If you set SECURE_SSL_REDIRECT to True, make sure the SECURE_REDIRECT_EXEMPT
...
")
      ...
      (comment "If you enable internal authentication, this is the address that the emails
...
")
      ...
      (name "server_email")
      (comment "If you enable email reporting for error messages, this is where those emails
will appear to be coming from. Make sure you set a valid domain name,
otherwise the emails may get rejected.
...
")
      ...
      (comment "On a production setup, setting COMPRESS_OFFLINE to True will bring a
...
")
  ...
  (mailman__web_default_configuration (list
      ...
      (value (list
          (list
            "Mailman Suite Admin"
            "root@" (jinja "{{ mailman__domain }}"))))
      
      (name "secret_key")
      (value (jinja "{{ ansible_local.mailman.secret_key | d(\"secretkey\") }}"))
      
      (name "mailman_archiver_key")
      (value (jinja "{{ ansible_local.mailman.archiver_key | d(\"restadmin\") }}"))
      
      (name "emailname")
      (value (jinja "{{ mailman__fqdn }}"))
      ...
      (config 
        (default 
          (ENGINE "django.db.backends.mysql")
          (NAME (jinja "{{ ansible_local.mailman.web_db_name | d(\"mailman3web\") }}"))
          (USER (jinja "{{ ansible_local.mailman.web_db_user | d(\"mailman3web\") }}"))
          (PASSWORD (jinja "{{ ansible_local.mailman.web_db_password | d(\"password\") }}"))
          (HOST (jinja "{{ ansible_local.mailman.web_db_host | d(\"localhost\") }}"))
          (PORT (jinja "{{ ansible_local.mailman.web_db_port | d(\"\") }}"))
          (OPTIONS 
            (init_command "SET sql_mode='STRICT_TRANS_TABLES'"))))
      ...
      (option "databases")
      (copy_id_from "installed_apps")
      (state (jinja "{{ \"present\" if (mailman__database_type == \"postgresql\") else \"ignore\" }}"))
      (config 
        (default 
      ...
      (value (jinja "{{ [\"mm\", \"identicon\", \"monsterid\", \"wavatar\", \"retro\", \"robohash\", \"pagan\"]
      ...
      (name "postorius_template_base_url")
      (value (jinja "{{ \"https://\" + mailman__fqdn + \"/\" }}"))
      
      (name "mailman_archiver_from")
      (value (jinja "{{ ([\"127.0.0.1\", \"::1\"]
               ...
               | sort | unique }}"))))
  (mailman__web_ldap_configuration (list
...
")
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))
      (copy_id_from "secret_key")
      (weight "-200")
      
      (name "auth_ldap_server_uri")
      (comment "Integration with the 'django-auth-ldap' library
https://django-auth-ldap.readthedocs.io/en/latest/reference.html
")
      (value (jinja "{{ mailman__ldap_uri | join(\" \") }}"))
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))
      
      (name "auth_ldap_start_tls")
      (value (jinja "{{ mailman__ldap_starttls }}"))
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))
      
      (name "auth_ldap_bind_dn")
      (value (jinja "{{ mailman__ldap_bind_dn }}"))
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))
      
      (name "auth_ldap_bind_password")
      (value (jinja "{{ mailman__ldap_bind_password }}"))
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))
      
      (name "auth_ldap_user_search")
      (raw "AUTH_LDAP_USER_SEARCH = LDAPSearch(
    '" (jinja "{{ mailman__ldap_people_dn }}") "',
    ldap.SCOPE_SUBTREE,
    '" (jinja "{{ mailman__ldap_people_filter }}") "',
)
")
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))
      
      (name "auth_ldap_group_search")
      (raw "AUTH_LDAP_GROUP_SEARCH = LDAPSearch(
    '" (jinja "{{ mailman__ldap_groups_dn }}") "',
    ldap.SCOPE_SUBTREE,
    '" (jinja "{{ mailman__ldap_groups_filter }}") "',
)
...
")
      ...
      (config 
        (is_staff (jinja "{{ mailman__ldap_superusers_group + \",\" + mailman__ldap_groups_dn }}"))
        (is_superuser (jinja "{{ mailman__ldap_superusers_group + \",\" + mailman__ldap_groups_dn }}")))
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))
      ...
      (state (jinja "{{ \"present\" if mailman__ldap_enabled | bool else \"ignore\" }}"))))
  (mailman__web_configuration (list))
  (mailman__web_group_configuration (list))
  (mailman__web_host_configuration (list))
  (mailman__web_combined_configuration (jinja "{{ mailman__web_original_configuration
