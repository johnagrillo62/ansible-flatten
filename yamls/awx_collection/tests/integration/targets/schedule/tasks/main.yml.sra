(playbook "awx_collection/tests/integration/targets/schedule/tasks/main.yml"
  (tasks
    (task "Generate a random string for test"
      (ansible.builtin.set_fact 
        (test_id (jinja "{{ lookup('password', '/dev/null chars=ascii_letters length=16') }}")))
      (when "test_id is not defined"))
    (task "Generate random string for schedule"
      (ansible.builtin.set_fact 
        (org_name "AWX-Collection-tests-organization-org-" (jinja "{{ test_id }}"))
        (sched1 "AWX-Collection-tests-schedule-sched1-" (jinja "{{ test_id }}"))
        (sched2 "AWX-Collection-tests-schedule-sched2-" (jinja "{{ test_id }}"))
        (cred1 "AWX-Collection-tests-schedule-cred1-" (jinja "{{ test_id }}"))
        (proj1 "AWX-Collection-tests-schedule-proj1-" (jinja "{{ test_id }}"))
        (proj2 "AWX-Collection-tests-schedule-proj2-" (jinja "{{ test_id }}"))
        (jt1 "AWX-Collection-tests-schedule-jt1-" (jinja "{{ test_id }}"))
        (jt2 "AWX-Collection-tests-schedule-jt1-" (jinja "{{ test_id }}"))
        (ee1 "AWX-Collection-tests-schedule-ee1-" (jinja "{{ test_id }}"))
        (label1 "AWX-Collection-tests-schedule-l1-" (jinja "{{ test_id }}"))
        (label2 "AWX-Collection-tests-schedule-l2-" (jinja "{{ test_id }}"))
        (ig1 "AWX-Collection-tests-schedule-ig1-" (jinja "{{ test_id }}"))
        (ig2 "AWX-Collection-tests-schedule-ig2-" (jinja "{{ test_id }}"))
        (slice_inventory "AWX-Collection-tests-schedule-slice-inv-" (jinja "{{ test_id }}"))
        (host_name "AWX-Collection-tests-schedule-host-" (jinja "{{ test_id }}"))
        (slice_num "10")))
    (task "Assert blocks"
      (block (list
          
          (name "Try to create without an rrule")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "present")
            (unified_job_template "Demo Job Template")
            (enabled "true"))
          (register "result")
          (ignore_errors "true")
          
          (name "Assert result is failed")
          (ansible.builtin.assert 
            (that (list
                "result is failed"
                "'Unable to create schedule '~ sched1 in result.msg")))
          
          (name "Create with options that the JT does not support")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "present")
            (unified_job_template "Demo Job Template")
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1")
            (description "This hopefully will not work")
            (extra_data 
              (some "var"))
            (inventory "Demo Inventory")
            (scm_branch "asdf1234")
            (job_type "run")
            (job_tags "other_tags")
            (skip_tags "some_tags")
            (limit "node1")
            (diff_mode "true")
            (verbosity "4")
            (enabled "true"))
          (register "result")
          (ignore_errors "true")
          
          (name "Unable to create schedule")
          (ansible.builtin.assert 
            (that (list
                "result is failed"
                "'Unable to create schedule '~ sched1 in result.msg")))
          
          (name "Build a real schedule")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "present")
            (unified_job_template "Demo Job Template")
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1"))
          (register "result")
          
          (name "Assert result is changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))
          
          (name "Use lookup to check that schedules was enabled")
          (ansible.builtin.set_fact 
            (schedules_enabled_test (jinja "{{lookup('awx.awx.controller_api', 'schedules/{{result.id}}/').enabled | bool}}")))
          
          (name "Newly created schedules should have API default value for enabled")
          (ansible.builtin.assert 
            (that (list
                "schedules_enabled_test")))
          
          (name "Build a real schedule with exists")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "exists")
            (unified_job_template "Demo Job Template")
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1"))
          (register "result")
          
          (name "Assert result did not change")
          (ansible.builtin.assert 
            (that (list
                "result is not changed")))
          
          (name "Delete a real schedule")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "absent")
            (unified_job_template "Demo Job Template")
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1"))
          (register "result")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))
          
          (name "Build a real schedule with exists")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "exists")
            (unified_job_template "Demo Job Template")
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1"))
          (register "result")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))
          
          (name "Rebuild the same schedule")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "present")
            (unified_job_template "Demo Job Template")
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1"))
          (register "result")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is not changed")))
          
          (name "Create a Demo Project")
          (project 
            (name (jinja "{{ proj1 }}"))
            (organization "Default")
            (allow_override "true")
            (state "present")
            (scm_type "git")
            (scm_url "https://github.com/ansible/ansible-tower-samples.git"))
          
          (name "Create a new organization")
          (organization 
            (name (jinja "{{ org_name }}")))
          
          (name "Create a Demo Project in another org")
          (project 
            (name (jinja "{{ proj2 }}"))
            (organization (jinja "{{ org_name }}"))
            (allow_override "true")
            (state "present")
            (scm_type "git")
            (scm_url "https://github.com/ansible/ansible-tower-samples.git"))
          
          (name "Create Credential1")
          (credential 
            (name (jinja "{{ cred1 }}"))
            (organization "Default")
            (credential_type "Red Hat Ansible Automation Platform"))
          (register "cred1_result")
          
          (name "Create Job Template with all prompts")
          (job_template 
            (name (jinja "{{ jt1 }}"))
            (organization "Default")
            (project (jinja "{{ proj1 }}"))
            (inventory "Demo Inventory")
            (playbook "hello_world.yml")
            (ask_variables_on_launch "true")
            (ask_inventory_on_launch "true")
            (ask_scm_branch_on_launch "true")
            (ask_credential_on_launch "true")
            (ask_job_type_on_launch "true")
            (ask_tags_on_launch "true")
            (ask_skip_tags_on_launch "true")
            (ask_limit_on_launch "true")
            (ask_diff_mode_on_launch "true")
            (ask_verbosity_on_launch "true")
            (ask_execution_environment_on_launch "true")
            (ask_forks_on_launch "true")
            (ask_instance_groups_on_launch "true")
            (ask_job_slice_count_on_launch "true")
            (ask_labels_on_launch "true")
            (ask_timeout_on_launch "true")
            (job_type "run")
            (state "present"))
          (register "result")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))
          
          (name "Create labels")
          (label 
            (name (jinja "{{ item }}"))
            (organization (jinja "{{ org_name }}")))
          (loop (list
              (jinja "{{ label1 }}")
              (jinja "{{ label2 }}")))
          
          (name "Create an execution environment")
          (execution_environment 
            (name (jinja "{{ ee1 }}"))
            (image "junk"))
          
          (name "Create instance groups")
          (instance_group 
            (name (jinja "{{ item }}")))
          (loop (list
              (jinja "{{ ig1 }}")
              (jinja "{{ ig2 }}")))
          
          (name "Create proper inventory for slice count")
          (inventory 
            (name (jinja "{{ slice_inventory }}"))
            (organization (jinja "{{ org_name }}"))
            (state "present"))
          (register "result")
          
          (name "Create a Host")
          (host 
            (name (jinja "{{ host_name }}") "-" (jinja "{{ item }}"))
            (inventory (jinja "{{ slice_inventory }}"))
            (state "present")
            (variables 
              (ansible_connection "local")))
          (loop (jinja "{{ range(slice_num)|list }}"))
          (register "result")
          
          (name "Create with options that the JT does support")
          (schedule 
            (name (jinja "{{ sched2 }}"))
            (state "present")
            (organization "Default")
            (unified_job_template (jinja "{{ jt1 }}"))
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1")
            (description "This hopefully will work")
            (extra_data 
              (some "var"))
            (inventory (jinja "{{ slice_inventory }}"))
            (scm_branch "asdf1234")
            (credentials (list
                (jinja "{{ cred1 }}")))
            (job_type "run")
            (job_tags "other_tags")
            (skip_tags "some_tags")
            (limit "node1")
            (diff_mode "true")
            (verbosity "4")
            (enabled "true")
            (execution_environment (jinja "{{ ee1 }}"))
            (forks "10")
            (instance_groups (list
                (jinja "{{ ig1 }}")
                (jinja "{{ ig2 }}")))
            (job_slice_count (jinja "{{ slice_num }}"))
            (labels (list
                (jinja "{{ label1 }}")
                (jinja "{{ label2 }}")))
            (timeout "10"))
          (register "result")
          (ignore_errors "true")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))
          
          (name "Reset some options")
          (schedule 
            (name (jinja "{{ sched2 }}"))
            (state "present")
            (execution_environment "")
            (forks "1")
            (instance_groups (list))
            (job_slice_count "1")
            (labels (list))
            (timeout "60"))
          (register "result")
          (ignore_errors "true")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))
          
          (name "Disable a schedule")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (unified_job_template "Demo Job Template")
            (state "present")
            (enabled "false"))
          (register "result")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))
          
          (name "Create a second Job Template in new org")
          (job_template 
            (name (jinja "{{ jt2 }}"))
            (project (jinja "{{ proj2 }}"))
            (inventory "Demo Inventory")
            (playbook "hello_world.yml")
            (job_type "run")
            (state "present"))
          
          (name "Build a schedule with a job template's name in two orgs")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "present")
            (unified_job_template (jinja "{{ jt2 }}"))
            (rrule "DTSTART:20191219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1"))
          (register "result")
          
          (name "Verify we can't find the schedule without the UJT lookup")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "present")
            (rrule "DTSTART:20201219T130551Z RRULE:FREQ=WEEKLY;INTERVAL=1;COUNT=1"))
          (register "result")
          (ignore_errors "true")
          
          (name "Assert result failed")
          (ansible.builtin.assert 
            (that (list
                "result is failed")))
          
          (name "Verify we can find the schedule with the UJT lookup and delete it")
          (schedule 
            (name (jinja "{{ sched1 }}"))
            (state "absent")
            (unified_job_template (jinja "{{ jt2 }}")))
          (register "result")
          
          (name "Assert result changed")
          (ansible.builtin.assert 
            (that (list
                "result is changed")))))
      (always (list
          
          (name "Delete the schedules")
          (schedule 
            (name (jinja "{{ item }}"))
            (state "absent"))
          (loop (list
              (jinja "{{ sched1 }}")
              (jinja "{{ sched2 }}")))
          (failed_when "false")
          
          (name "Delete the jt1")
          (job_template 
            (name (jinja "{{ jt1 }}"))
            (project (jinja "{{ proj1 }}"))
            (playbook "hello_world.yml")
            (state "absent"))
          (register "del_res")
          (until "del_res is succeeded")
          (retries "5")
          (delay "3")
          
          (name "Delete the jt2")
          (job_template 
            (name (jinja "{{ jt2 }}"))
            (project (jinja "{{ proj2 }}"))
            (playbook "hello_world.yml")
            (state "absent"))
          (register "del_res")
          (until "del_res is succeeded")
          (retries "5")
          (delay "3")
          
          (name "Delete the Project2")
          (project 
            (name (jinja "{{ proj2 }}"))
            (organization (jinja "{{ org_name }}"))
            (state "absent")
            (scm_type "git")
            (scm_url "https://github.com/ansible/ansible-tower-samples.git"))
          (register "del_res")
          (until "del_res is succeeded")
          (retries "5")
          (delay "3")
          (failed_when "false")
          
          (name "Delete the Project1")
          (project 
            (name (jinja "{{ proj1 }}"))
            (organization "Default")
            (state "absent")
            (scm_type "git")
            (scm_url "https://github.com/ansible/ansible-tower-samples.git"))
          (register "del_res")
          (until "del_res is succeeded")
          (retries "5")
          (delay "3")
          
          (name "Delete Credential1")
          (credential 
            (name (jinja "{{ cred1 }}"))
            (organization "Default")
            (credential_type "Red Hat Ansible Automation Platform")
            (state "absent"))
          (failed_when "false")
          
          (name "Delete an execution environment")
          (execution_environment 
            (name (jinja "{{ ee1 }}"))
            (image "junk")
            (state "absent"))
          (failed_when "false")
          
          (name "Delete instance groups")
          (instance_group 
            (name (jinja "{{ item }}"))
            (state "absent"))
          (loop (list
              (jinja "{{ ig1 }}")
              (jinja "{{ ig2 }}")))
          (failed_when "false")
          
          (name "Remove the organization")
          (organization 
            (name (jinja "{{ org_name }}"))
            (state "absent"))
          (failed_when "false")
          
          (name "Delete slice inventory")
          (inventory 
            (name (jinja "{{ slice_inventory }}"))
            (organization (jinja "{{ org_name }}"))
            (state "absent"))
          (failed_when "false")
          
          (name "Delete slice hosts")
          (host 
            (name (jinja "{{ host_name }}") "-" (jinja "{{ item }}"))
            (inventory (jinja "{{ slice_inventory }}"))
            (state "absent"))
          (loop (jinja "{{ range(slice_num)|list }}"))
          (failed_when "false"))))))
